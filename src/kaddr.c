...
#if 0
bool kgl_addr_build(struct addrinfo  *addr, uint16_t port, sockaddr_i *sockaddr)
{
	kgl_memcpy(sockaddr, addr->ai_addr, KGL_MIN(addr->ai_addrlen, sizeof(sockaddr_i)));
#ifdef KSOCKET_IPV6
...
static kev_result addr_next_call(KOPAQUE data, void *arg, int len)
{
...
static void addr_result(kgl_addr_queue *queue, kgl_addr *addr) {
	...
	while (queue) {
		...
		kgl_selector_module.next(queue->selector, NULL, addr_next_call, queue, 0);
		queue = next;
	}
}
...
kev_result kgl_addr_timer(KOPAQUE data, void *arg, int got)
{
...
static kev_result when_selector_manager_ready(KOPAQUE data, void* arg, int got)
{
	kselector *selector = get_perfect_selector();
	kassert(selector);
	kgl_selector_module.next(selector,NULL, kgl_addr_timer, selector, 0);
	return kev_ok;
}
void kgl_addr_init()
{
