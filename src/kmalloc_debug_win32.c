...
#define SKIP_STACK_TRACE_COUNT  1
...
void print_stack_trace(TRACEBACK sp)
{
	...
	for (int j = 0; j < MAX_TRACEBACK_LEVELS; j++) {
		...
		printf("\t{%p}", sp[j]);
		if (SymGetLineFromAddr(hProcess, (DWORD64)sp[j], &displacement, &sourceinfo)) {
			printf(" %s:%d", sourceinfo.FileName, sourceinfo.LineNumber);
		}
		printf("\n");
	}
}
...
void kgl_get_stack_trace(TRACEBACK sp)
{
	CaptureStackBackTrace(SKIP_STACK_TRACE_COUNT, MAX_TRACEBACK_LEVELS, sp, NULL);
	return;
}
...
void win32_malloc_hook(long request, size_t size, const char *file, int line)
{
	...
	kgl_get_stack_trace(ptr->where_alloced);
	int new_flag = 0;
